...
interface AuthProps {
  onLogin: (data?: { name: string; email: string; companyName: string }) => void;
  onBack: () => void;
}
...
const Auth: React.FC<AuthProps> = ({ onLogin, onBack }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isLogin) {
        // Login Flow - In a real app we'd fetch the user profile here.
        // For this demo, we assume the default mock user or previously saved local user.
        onLogin({ 
            email: email,
            name: 'Alex Accountant', // Fallback for login simulation
            companyName: 'Finance Pro Ltd.' // Fallback for login simulation
        });
    } else {
  ...
  const handleVerifyCode = (e: React.FormEvent) => {
      ...
      setTimeout(() => {
          ...
          if (verificationCode.length >= 4) {
              // Pass the registered data back to App.tsx
              onLogin({
                  email: email,
                  name: fullName,
